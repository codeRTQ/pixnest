...
- **文件夹导入**：`📁 选择整个文件夹导入`（或直接拖文件夹进来）→ 按文件名排序（`00001→00041`）、
  自动用文件夹名填标题（填了「模特」会自动剥掉前缀）、过滤非图片
- **批量导入多个文件夹**（`/batch`）：**按模特 / 系列批量导入** —— 每次先填本批的模特或系列
  （会套用到本批全部图集），再选中该模特（或系列）目录，里面每个子文件夹自动成为一套图集。
  标题默认取文件夹名，填了模特会自动剥掉文件夹名开头的模特名；逐套串行上传并显示进度，
  全部完成后统一重建一次（避免逐套重建）
  - **模特自动识别**：从文件夹路径推断（图集目录的上一层），可手动改；上次填的值会记住
  - **层级自动识别**：选父文件夹（`根/图集/文件`）与拖入图集目录（`图集/文件`）都能正确分组，
    多层结构可手动指定第 1/2/3 层
  - **重复导入安全**：图片按内容 MD5 去重，同一天+同标题再导一次 = 只补新增、不覆盖不重复，
    新图续号命名（`04.jpg`、`05.jpg`）不会撞名；**换日期导入会新建一套**（标题相同 = 两套并存）
- 缩略图**并发生成**（`THUMB_WORKERS`，默认 min(8, 逻辑核数)）+ JPEG `draft()` 快速降采样解码
  → 实测 8 核机器 41 张原图：单线程 29s → 8 线程 5.5s（约 9 倍）
- **网盘链接批量导入**（`/links`）：粘贴 `关键词 | 网盘名 | 链接 | 提取码` 多行 →
  自动匹配图集（支持 `NO.001` 编号与中文片段、逗号/制表符分隔、`?pwd=` 与「提取码: xxxx」）→
  预览匹配结果 → 一键写入并重建
- 自动生成缩略图（**长边 1920px**，横竖构图总像素相近：竖 1280×1920 / 横 1920×1280，比例与原图一致
  + LQIP + WebP）、封面（800×1067）
...
- 模特资料、网盘外链、网盘提取码、解压密码等字段（留空则不显示）
...
├── images/       # 原图
├── thumbs/       # 缩略图 长边1920px(.webp 主 + .jpg 回退) + .lqip.jpg（后台自动生成）
