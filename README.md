...
- **批量导入多个文件夹**（`/batch`）：选「装着多套图集的上一级目录」或一次拖入多个文件夹 →
  **每个子文件夹自动成为一套图集**，标题默认取文件夹名，系列/模特/标签可设默认值批量套用；
  逐套串行上传并显示进度，全部完成后统一重建一次（避免逐套重建）
  - 多级目录：用「每套图集位于选中目录下的第几层」控制分组层级（默认第 1 层）
